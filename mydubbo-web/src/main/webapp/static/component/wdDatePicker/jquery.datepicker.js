(function($) {
	...
	$.fn.datepicker = function(o) {
		var def = {
			weekStart : 0,
			weekName : [ i18n.datepicker.dateformat.sun, i18n.datepicker.dateformat.mon, i18n.datepicker.dateformat.tue, i18n.datepicker.dateformat.wed, i18n.datepicker.dateformat.thu, i18n.datepicker.dateformat.fri, i18n.datepicker.dateformat.sat ], // week
			// language
			// support
		...
		function returndate() {
			...
			$(ct).triggerHandler("change", [ ct.val(), ct ]);
		...
		function validator(ct) {
			if (def.validator) {
				def.validator.element(ct);
			}
		}
		;
		function writecb() {
		...
		return $(this).each(function() {
			...
			if ($(obj).next().hasClass("picker")) {
				def.showtarget == null;
			} else {
				def.showtarget == null && obj.after(picker);
			}
			//设置为只读
			$(obj).attr("readonly", def.readonly);
			...
			picker.click(function(e) {
				...
				for ( var type in def.listeners) {
					var handler = def.listeners[type];
				...
				var height = $(t).outerHeight(true);// edited by yangFan
				// 2013年12月12日 15:24:40
				var newpos = {
					left : pos.left,
					top : pos.top + height
				};
